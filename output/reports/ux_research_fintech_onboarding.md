...
### Progressive Tab Navigation Pattern
**Context:** Resolves tension between exposing powerful features early and overwhelming new users with too many choices simultaneously.

**Options Considered:**
- Bottom tab bar visible from app launch always
- Tabs revealed progressively as onboarding milestones complete

**Chosen Approach:** Unlock navigation tabs incrementally as Marcus completes each onboarding milestone, starting with two core tabs.

**Rationale:** Hick's Law dictates that reducing initial choices speeds decision-making, directly addressing Marcus Chen's need for immediate directional clarity without cognitive overload.

**Trade-offs:** Power users like Marcus Chen may feel patronized by gated navigation, requiring clear visual affordances indicating locked tabs are temporary.

**Success Metric:** Time-to-first-meaningful-action under 90 seconds for 70% of new users in usability testing.

### Value-First Deferred KYC Flow
**Context:** Resolves the premature KYC prompt pain point by demonstrating product value before requesting sensitive personal information.

**Options Considered:**
- Collect full KYC data before any app exploration permitted
- Allow read-only portfolio exploration before triggering KYC prompt

**Chosen Approach:** Let users explore simulated portfolio and rate data for up to 10 minutes before surfacing a contextual KYC prompt tied to a specific action.

**Rationale:** Progressive Disclosure supports deferring KYC complexity until Roy Tanner has built enough precarious trust to willingly share sensitive data.

**Trade-offs:** Deferred KYC increases drop-off risk at the verification step for users who explore but never commit, inflating acquisition cost without conversion.

**Success Metric:** KYC completion rate above 60% among users who reach the deferred prompt within the first session.

### Inline Contextual Error Handling
**Context:** Resolves Diane's frustration with form errors appearing only after full submission, breaking her high-velocity input behavior.

**Options Considered:**
- Show all validation errors only on form submission tap
- Validate each field inline immediately after user leaves focus

**Chosen Approach:** Validate fields on blur with inline micro-copy that explains the fix in plain language beneath the offending field.

**Rationale:** Postel's Law—accepting liberal user input and responding with precise, forgiving feedback—directly supports Diane Kowalski's need for speed without punitive re-entry loops.

**Trade-offs:** Aggressive inline validation on fast-typing users can produce flickering error states that feel accusatory before input is complete.

**Success Metric:** Form error recovery rate above 85% on first correction attempt without requiring full re-entry.

### Chunked Onboarding Information Architecture
**Context:** Resolves the cognitive overload of presenting all account setup requirements on a single dense screen.

**Options Considered:**
- Single scrollable setup screen with all fields visible at once
- Seven discrete screens each containing one to three related fields

**Chosen Approach:** Group onboarding steps into chunks of no more than three related fields per screen with a persistent progress indicator.

**Rationale:** Miller's Law—working memory capacity of roughly seven plus or minus two items—justifies chunking to prevent Roy Tanner from abandoning setup due to perceived complexity.

**Trade-offs:** More screens increases total perceived length of onboarding, which risks frustrating Marcus Chen who prioritizes reaching the core product quickly.

**Success Metric:** Onboarding abandonment rate below 20% across all seven screens measured via funnel analytics.

### Trust-Anchored Security Disclosure UX
**Context:** Resolves Roy Tanner's precarious trust state by making encryption and data practices visible before he is asked to input any data.

**Options Considered:**
- Show security certifications only in Settings or footer links
- Surface a single prominent trust badge and one-line security statement on the first data-entry screen

**Chosen Approach:** Display a Von Restorff-isolated trust badge with a one-sentence encryption statement directly above the first personal data field.

**Rationale:** The Von Restorff Effect ensures the isolated trust badge is memorably distinct, directly countering Roy Tanner's precarious trust state at the highest-anxiety moment of data entry.

**Trade-offs:** Prominent security framing may signal to lower-anxiety users like Marcus Chen that the app is overly cautious, potentially undermining a perception of seamless modernity.

**Success Metric:** Trust perception score above 4.0 out of 5.0 on post-onboarding survey question about data safety confidence.

### Eliminate Branding Splash Animation
**Context:** Resolves the pain point of wasted interactive time caused by a mandatory branding animation before the app becomes usable.

**Options Considered:**
- Retain full 3-second branded splash animation on every cold launch
- Replace splash with an instant-render skeleton screen loading real content beneath the logo

**Chosen Approach:** Replace the splash animation with a static logo visible for no more than 400 milliseconds while content hydrates behind a skeleton screen.

**Rationale:** The Doherty Threshold—system responses under 400ms maintain user flow state—ensures Marcus Chen experiences immediate interactivity and perceives the app as fast and responsive.

**Trade-offs:** Removing the branded animation reduces a deliberate aesthetic moment that the Aesthetic-Usability Effect could leverage to build early emotional goodwill with Diane Kowalski.

**Success Metric:** Perceived load time rated as fast or very fast by at least 80% of users in post-session usability interviews.
...
**ProgressiveTabBar** — Priority: `must-have`
Variants: two-tab-locked, three-tab-unlocked, full-unlocked
Used in: Onboarding Flow & Profile Setup, First Use & Feature Exploration, Regular Use & Advocacy
UX Principles: Progressive Disclosure, Zeigarnik Effect

**ChunkedStepIndicator** — Priority: `must-have`
Variants: dot-stepper, segmented-bar
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: Goal Gradient Effect, Miller's Law

**BackNavigationHeader** — Priority: `must-have`
Variants: back-only, back-with-exit
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: User Control and Freedom

**ContextualKYCPromptBanner** — Priority: `must-have`
Variants: bottom-sheet-trigger, inline-action-interrupt
Used in: First Use & Feature Exploration, KYC Completion (Deferred Return)
UX Principles: Peak-End Rule, Timing of Persuasion

### Forms

**InlineValidatedInput** — Priority: `must-have`
Variants: text, email, phone, password, numeric
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: Inline Validation, Error Prevention, Plain Language

**TrustAnchoredFieldGroup** — Priority: `must-have`
Variants: single-trust-badge, grouped-fields-with-badge
Used in: Account Creation & Identity Verification, KYC Completion
UX Principles: Von Restorff Effect, Trust Signaling

**DocumentUploadField** — Priority: `must-have`
Variants: camera-capture, file-upload, liveness-check
Used in: KYC Completion (Deferred Return)
UX Principles: Affordance, Error Prevention

**PrimaryActionButton** — Priority: `must-have`
Variants: full-width, inline, loading-state
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: Fitts's Law, Visual Hierarchy

### Feedback

**SkeletonScreen** — Priority: `must-have`
Variants: card-skeleton, list-skeleton, full-page-skeleton
Used in: Download & Initial Launch, First Use & Feature Exploration
UX Principles: Perceived Performance, Doherty Threshold

**InlineFieldError** — Priority: `must-have`
Variants: micro-copy-error, warning-hint
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: Error Recovery, Plain Language Principle

**MilestoneToast** — Priority: `should-have`
Variants: tab-unlock-celebration, kyc-success, profile-complete
Used in: Onboarding Flow & Profile Setup, KYC Completion, Regular Use & Advocacy
UX Principles: Positive Reinforcement, Peak-End Rule

**SimulatedDataDisclaimer** — Priority: `must-have`
Variants: banner, tooltip-tag
Used in: First Use & Feature Exploration
UX Principles: Transparency, Trust Calibration

### Content

**StaticLogoSplash** — Priority: `must-have`
Variants: light-theme, dark-theme
Used in: Download & Initial Launch
UX Principles: Eliminate Unnecessary Animation, Perceived Performance

**ValuePropositionCard** — Priority: `must-have`
Variants: icon-headline-body, illustration-headline
Used in: Awareness & Discovery, Download & Initial Launch, Onboarding Flow & Profile Setup
UX Principles: Serial Position Effect, Emotional Design

**TrustBadge** — Priority: `must-have`
Variants: encryption-statement, regulatory-badge
Used in: Account Creation & Identity Verification, KYC Completion
UX Principles: Von Restorff Effect, Trust Signaling

**SimulatedPortfolioCard** — Priority: `must-have`
Variants: rate-display, portfolio-summary
Used in: First Use & Feature Exploration
UX Principles: Endowment Effect, Value-First Principle

### Layout

**ChunkedFormScreen** — Priority: `must-have`
Variants: one-column, scrollable
Used in: Account Creation & Identity Verification, Onboarding Flow & Profile Setup, KYC Completion
UX Principles: Miller's Law, Chunking

**BottomSheetModal** — Priority: `must-have`
Variants: half-sheet, full-sheet, drag-dismissible
Used in: First Use & Feature Exploration, KYC Completion (Deferred Return)
UX Principles: Progressive Disclosure, User Control

**SafeAreaPageWrapper** — Priority: `must-have`
Variants: with-tab-bar, without-tab-bar, modal-context
Used in: Download & Initial Launch, Account Creation & Identity Verification, Regular Use & Advocacy
UX Principles: Consistency, Platform Conventions

**SectionDivider** — Priority: `should-have`
Variants: label-divider, spacer-only
Used in: Onboarding Flow & Profile Setup, Account Creation & Identity Verification
UX Principles: Visual Grouping, Gestalt Proximity

### Design Tokens
**Color Roles:** primary-action, destructive, success, warning, neutral, surface, on-surface, trust-accent, simulated-data-tint
**Type Scale:** display, heading-1, heading-2, body, body-emphasis, caption, caption-emphasis
**Spacing:** 8px base unit

---

## Stage 8 — Low-fi Wireframes

### Welcome Splash
**Purpose:** Brand introduction and value proposition display
**Layout:** single-column

**Wireframe Description:**
Dark full-screen layout with centered logo and value prop card. Trust badge and CTA anchored near bottom.

**Components Used:** StaticLogoSplash, ValuePropositionCard, TrustBadge, PrimaryActionButton

**Annotations:** Auto-advance to Account Creation after 0.5s if returning user token found

### Account Creation
**Purpose:** Collect basic user credentials to create account
**Layout:** form

**Wireframe Description:**
White form screen with step indicator at top. Three validated inputs stacked with trust badge above sticky CTA.

**Components Used:** BackNavigationHeader, ChunkedStepIndicator, TrustAnchoredFieldGroup, InlineValidatedInput, InlineFieldError, TrustBadge, PrimaryActionButton

**Annotations:** Inline validation fires on blur, not on submit

### Profile Setup
**Purpose:** Collect financial profile and preferences
**Layout:** form

**Wireframe Description:**
Light-gray scrollable form with chunked sections. Simulated portfolio card previews outcome before KYC.

**Components Used:** BackNavigationHeader, ChunkedStepIndicator, ChunkedFormScreen, InlineValidatedInput, SectionDivider, SimulatedDataDisclaimer, SimulatedPortfolioCard, PrimaryActionButton

**Annotations:** Simulated portfolio card updates live as risk tolerance changes

### Dashboard Preview
**Purpose:** Show app value before KYC with deferred prompt
**Layout:** tab-bar

**Wireframe Description:**
Full dashboard with simulated data and sticky KYC banner. Tab bar unlocks progressively post-verification.

**Components Used:** ContextualKYCPromptBanner, SkeletonScreen, SimulatedPortfolioCard, SimulatedDataDisclaimer, SectionDivider, MilestoneToast, ProgressiveTabBar

**Annotations:** Milestone toast auto-dismisses after 3 seconds

### KYC Verification
**Purpose:** Collect identity documents to complete KYC
**Layout:** form

**Wireframe Description:**
Three stacked document upload fields with trust anchors. Sticky CTA submits async and returns to dashboard.

**Components Used:** BackNavigationHeader, ChunkedStepIndicator, TrustAnchoredFieldGroup, DocumentUploadField, TrustBadge, BottomSheetModal, PrimaryActionButton, SafeAreaPageWrapper

**Annotations:** Submit triggers async review; user returns to dashboard immediately
